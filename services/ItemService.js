  ...
  return Object.keys(bucketItems)
    .map(characterID => {
      return bucketItems[characterID]
        ? Math.ceil(
            bucketItems[characterID].length *
              52 /
              (characterID === 'vault' ? vaultColumns : 3) +
              52
          )
        : 0
    })
...
export default function(getBungieRequest) {
  return {
    ...
    equipItem(itemId, characterId) {
      return getBungieRequest().then(bungieRequest =>
        bungieRequest.equipItem(itemId, characterId)
      )
    },
    ...
    getCharacter(characterID, characterMembershipID) {
      return getBungieRequest().then(bungieRequest =>
        bungieRequest
          .getCharacterById(characterID, characterMembershipID)
          .then(({ data }) => data)
      )
    },
            ...
            ({
              ...
              return inventory.data.items
                ...
                .map(rawItem => {
                  ...
                  const stats =
                    itemComponents.stats.data[rawItem.itemInstanceId]
                  const perks =
                    itemComponents.perks.data[rawItem.itemInstanceId]
