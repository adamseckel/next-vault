...
class ItemRow extends Component {
  ...
  handleMouseMove = ({ pageX, pageY }) => {
    ...
    if (isPressed) {
      ...
      const characterID = this.props.characters[
        Object.keys(this.props.characters)[characterIndex]
      ]
      ...
      const columns =
        Object.keys(this.props.characters).length * 4 + this.props.vaultColumns

      const col = clamp(
        Math.floor(mouseXY[0] / width - characterIndex),
        0,
        columns
      )
      ...
      const finalFinalCol =
        row && characterID !== 'vault' ? finalCol - 2 : finalCol
      const index =
        row * (characterID === 'vault' ? this.props.vaultColumns : 4) +
        finalFinalCol

      const currentIndexItem = characterItems[index]
      const newIndex =
        order.indexOf(currentIndexItem) === -1
          ? order.length
          : order.indexOf(currentIndexItem)

      try {
        const proposedOrder = reinsert(
          order,
          characterID,
  ...
  handleMouseUp = () => {
    ...
    const { itemInstanceId, itemHash } = this.state.items[
      this.state.lastItem.id
    ]
    ...
    if (
      (this.state.lastCharacter === 'vault' ||
        (!shouldEquip && !shouldUnequip)) &&
  ...
  handleToggleSort = () => {
    ...
    const newSortIndex =
      currentSortIndex === sortByOptions.length - 1 ? 0 : currentSortIndex + 1
    const sortBy = sortByOptions[newSortIndex]
    const nextSortIndex =
      newSortIndex === sortByOptions.length - 1 ? 0 : newSortIndex + 1
    ...
    const equippedItems = Object.keys(this.props.characters).map(
      characterID => {
        return this.state.order.filter(
          order => order.characterID === characterID
        )[0]
      }
    )
    ...
    const orderedInventory = _.sortBy(
      this.state.order,
      item => item[sort]
    ).filter(item => {
    ...
    const newOrder = equippedItems.concat(
      negative ? orderedInventory.reverse() : orderedInventory
    )
    this.setState({
      sortBy,
      nextSortName,
  ...
  renderBucket(items, characterId, layout, order, query) {
    ...
    return (
      <RowWithMargin align="space-between" justify="start">
        <InventoryBucket
