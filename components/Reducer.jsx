...
class Reducer extends Component {
  ...
  setMembership = (
    ...
    // try {
    //   this.state.firebaseService.insertOrUpdateUserAndTrackVisit(
    //     membership.bungieNetUser
    //   )
    // } catch (e) {
    //   this.state.firebaseService.trackError(
    //     `Failed to track visist: ${e.message}`
    //   )
    // }
    ...
    //     console.log('incrementing accounts')
    //     return this.setMembership(
    //       membership,
    //       authorization,
    //       manifestRequestService,
  ...
  getAccount = (destinyMembership, membership) => {
    return Promise.all([
      ...
      const characterArray = Object.keys(characters).map(
        characterId => characters[characterId]
      )
      const vaultColumns = calculateVaultColumns(
        characterArray,
      ...
      this.setState({
        ...
        charactersByID,
        vaultColumns,
        inventoryDefinitions: DestinyInventoryItemDefinition,
        bucketDefinitions: DestinyInventoryBucketDefinition,
  ...
  startInventoryPolling = () => {
    if (
      ...
      this.state.inventoryPolling ||
      this.state.disablePolling
    )
      return
  ...
  inventoryPoll = (count, instance) => {
    ...
    const pollDelay =
      count > ppm * 5
        ? count / (ppm * 5) * basePollingInterval
        : basePollingInterval
    ...
    inventoryPollingInterval = setTimeout(() => {
      ...
      // } catch (e) {
      //   this.state.firebaseService.trackError(
      //     `Failed to track poll: ${e.message}`
      //   )
      // }
  ...
  getItemDetail = (characterID, itemInstanceID) => {
    ...
    return ItemService(this.authorize).getItemDetail(
      membershipId,
      characterID,
      itemInstanceID
    )
  ...
  createTransferPromise = (
    ...
    try {
      ...
      // this.state.firebaseService.trackError(
      //   `Failed to track transfer: ${e.message}`
      // )
    }
    ...
    if (
      shouldUnequipReplacementItemID &&
      lastCharacterID === initialCharacterID
    ) {
      return ItemService(this.authorize).equipItem(
        shouldUnequipReplacementItemID,
        lastCharacterID
      )
    }
        ...
        return fromVault && shouldEquip
          ...
          : !isSpecificVaultTransaction
            ? ItemService(this.authorize)
                ...
                .then(result => {
                  return shouldEquip
                    ? ItemService(this.authorize).equipItem(
                        itemId,
                        lastCharacterID
                      )
                    : result
  ...
  updateCharacters = membershipID => {
    return ItemService(this.authorize)
      ...
      .then(characters => {
        this.setState({
          characters: Object.keys(characters).map(
            characterId => characters[characterId]
          ),
  ...
  updateWidth = () => {
    this.setState({
      ...
      vaultColumns: calculateVaultColumns(
        this.state.characters,
        this.refs.client.clientWidth
      ),
    })
  }
